...
(function () {
  ...
  // اگه کاربر با حساب واقعی (Firebase Auth) وارد و ایمیلش تایید شده باشه،
  // هویت واقعی‌ش برای ارسال پیام/بن استفاده می‌شه؛ وگرنه شناسه‌ی محلی مهمان.
  function isAuthedVerified() {
    return !!(window.AUTH_STATE && window.AUTH_STATE.user && window.AUTH_STATE.verified);
  }

  function activeUid() {
    return isAuthedVerified() ? window.AUTH_STATE.user.uid : uid;
  }

  function activeName() {
    if (isAuthedVerified() && window.AUTH_STATE.profile && window.AUTH_STATE.profile.displayName) {
      return window.AUTH_STATE.profile.displayName;
    }
    return getName();
  }
  ...
  function computeAvailability() {
    ...
    if (!isAuthedVerified()) {
      const notice = (window.AUTH_STATE && window.AUTH_STATE.user)
        ? "⚠️ برای ارسال پیام، اول باید ایمیلت رو تایید کنی (دکمه‌ی «ورود» بالای صفحه، لینک تایید رو دوباره بفرست)."
        : "🔒 برای ارسال پیام باید وارد حساب بشی. از دکمه‌ی «ورود» بالای صفحه ثبت‌نام یا وارد شو.";
      return { ok: false, canRead: true, notice };
    }
  ...
  // کش پروفایل کاربران (برای نمایش نشان تیک‌آبی/VIP کنار اسم توی پیام‌ها)
  state.usersCache = {};
  function listenUsersCache() {
    ensureFirebase().ref("users").on("value", (snap) => {
  ...
  function listenMessages() {
    ensureFirebase().ref("chat/messages").limitToLast(state.pageSize).on("value", (snap) => {
      ...
      Object.entries(val).forEach(([id, m]) => {
        if (!prevIds.has(id) && m.uid !== activeUid() && m.time > state.lastSeenTime) {
          if (!state.panelOpen) {
            state.unread++;
            playNotifySound();
  ...
  function renderOneMessage(id, m) {
    ...
    row.className = "chat-msg-row" + (m.uid === activeUid() ? " mine" : "") + (m.isAdmin ? " admin-msg" : "");
    ...
    row.innerHTML = `
      <div class="chat-msg-top">
        <span class="chat-msg-name pf-clickable-name" data-open-profile="${m.uid}">${escapeHtml(m.name || "ناشناس")}</span>
        ${(state.usersCache[m.uid] && state.usersCache[m.uid].badge && window.Profile) ? window.Profile.badgeHtml(state.usersCache[m.uid].badge) : ""}
      ...
      <div class="chat-msg-actions">
        ...
        ${m.uid !== activeUid() ? '<span data-act="pm">✉️ پیام خصوصی</span>' : ""}
        ${m.uid === activeUid() ? '<span data-act="edit">✏️ ویرایش</span><span data-act="delete">🗑 حذف</span>' : ""}
      </div>
    ...
    row.querySelectorAll('[data-act="pm"]').forEach(el => el.onclick = () => {
      if (window.PrivateChat) window.PrivateChat.openWith(m.uid, m.name);
    });
    row.querySelectorAll('[data-open-profile]').forEach(el => el.onclick = () => {
      if (window.Profile) window.Profile.open(el.getAttribute("data-open-profile"));
    });
  ...
  function init() {
    ...
    // وقتی وضعیت لاگین/تایید ایمیل عوض بشه (login/logout/verify)، اجازه‌ی ارسال پیام هم آپدیت بشه
    if (window.Auth && window.Auth.onAuthChange) {
      window.Auth.onAuthChange(() => refreshAvailabilityUi());
    }
    listenTyping();
  }
